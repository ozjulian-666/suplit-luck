...
// ── Acción: finalizar rifa (sorteo aleatorio + notificar ganador) ─────────────
$msg_accion = "";
$resultado_sorteo = null; // para mostrar el resultado en pantalla

if (isset($_GET["accion"]) && $_GET["accion"] === "finalizar" && isset($_GET["rid"])) {
    $rid = (int)$_GET["rid"];
    if ($rid > 0) {
        // Obtener info de la rifa
        $stmt_info = mysqli_prepare($conexion, "SELECT id_organizador, titulo, premio FROM rifas WHERE id = ? AND estado = 'activa'");
        ...
        if ($rifa_info) {
            // Obtener TODAS las boletas vendidas de esta rifa
            $stmt_bol = mysqli_prepare($conexion,
                "SELECT b.id, b.numero, b.id_usuario, u.nombre, u.apellido
                 FROM boletas b
                 JOIN usuarios u ON u.id = b.id_usuario
                 WHERE b.id_rifa = ? AND b.estado = 'vendida'"
            );
            mysqli_stmt_bind_param($stmt_bol, "i", $rid);
            mysqli_stmt_execute($stmt_bol);
            $boletas_vendidas = mysqli_fetch_all(mysqli_stmt_get_result($stmt_bol), MYSQLI_ASSOC);
            mysqli_stmt_close($stmt_bol);

            if (!empty($boletas_vendidas)) {
                // Seleccionar ganador aleatorio
                $ganadora = $boletas_vendidas[array_rand($boletas_vendidas)];
                $id_boleta_ganadora  = (int)$ganadora["id"];
                $numero_ganador      = (int)$ganadora["numero"];
                $id_usuario_ganador  = (int)$ganadora["id_usuario"];
                $nombre_ganador      = $ganadora["nombre"] . " " . $ganadora["apellido"];
            } else {
                // Sin boletas vendidas: sortear número aleatorio de la rifa
                $stmt_num = mysqli_prepare($conexion, "SELECT numero FROM boletas WHERE id_rifa = ? ORDER BY RAND() LIMIT 1");
                mysqli_stmt_bind_param($stmt_num, "i", $rid);
                mysqli_stmt_execute($stmt_num);
                $row_num = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt_num));
                mysqli_stmt_close($stmt_num);
                $id_boleta_ganadora = null;
                $numero_ganador     = $row_num ? (int)$row_num["numero"] : 0;
                $id_usuario_ganador = null;
                $nombre_ganador     = "Sin participantes";
            }

            mysqli_begin_transaction($conexion);
            try {
                // 1. Marcar rifa como finalizada
                ...
                // 2. Registrar en tabla sorteos
                $stmt_sorteo = mysqli_prepare($conexion,
                    "INSERT INTO sorteos (id_rifa, fecha_sorteo, numero_ganador, estado) VALUES (?, NOW(), ?, 'ejecutado')"
                );
                mysqli_stmt_bind_param($stmt_sorteo, "ii", $rid, $numero_ganador);
                mysqli_stmt_execute($stmt_sorteo);
                $id_sorteo_nuevo = mysqli_insert_id($conexion);
                mysqli_stmt_close($stmt_sorteo);

                // 3. Registrar en tabla ganadores (solo si hubo comprador)
                if ($id_usuario_ganador && $id_boleta_ganadora) {
                    $stmt_gan = mysqli_prepare($conexion,
                        "INSERT INTO ganadores (id_sorteo, id_usuario, id_boleta) VALUES (?, ?, ?)"
                    );
                    mysqli_stmt_bind_param($stmt_gan, "iii", $id_sorteo_nuevo, $id_usuario_ganador, $id_boleta_ganadora);
                    mysqli_stmt_execute($stmt_gan);
                    mysqli_stmt_close($stmt_gan);

                    // 4. Notificación al GANADOR en tabla notificaciones (estructura real de la BD)
                    $msg_ganador = "¡Felicitaciones! Ganaste la rifa "" . $rifa_info["titulo"] . "". Tu boleta #" . str_pad($numero_ganador, 4, "0", STR_PAD_LEFT) . " fue seleccionada como ganadora. Premio: " . $rifa_info["premio"] . ". Contacta al organizador para reclamar tu premio.";
                    $stmt_noti_gan = mysqli_prepare($conexion,
                        "INSERT INTO notificaciones (id_usuario, id_rifa, mensaje, tipo) VALUES (?, ?, ?, 'resultado')"
                    );
                    mysqli_stmt_bind_param($stmt_noti_gan, "iis", $id_usuario_ganador, $rid, $msg_ganador);
                    mysqli_stmt_execute($stmt_noti_gan);
                    mysqli_stmt_close($stmt_noti_gan);

                    // 5. Notificación al ORGANIZADOR
                    $id_org = (int)$rifa_info["id_organizador"];
                    $msg_org = "Tu rifa "" . $rifa_info["titulo"] . "" ha finalizado. Ganador: " . $nombre_ganador . " con la boleta #" . str_pad($numero_ganador, 4, "0", STR_PAD_LEFT) . ".";
                    $stmt_noti_org = mysqli_prepare($conexion,
                        "INSERT INTO notificaciones (id_usuario, id_rifa, mensaje, tipo) VALUES (?, ?, ?, 'resultado')"
                    );
                    mysqli_stmt_bind_param($stmt_noti_org, "iis", $id_org, $rid, $msg_org);
                    mysqli_stmt_execute($stmt_noti_org);
                    mysqli_stmt_close($stmt_noti_org);
                }
                ...
                $resultado_sorteo = [
                    ...
                    "premio"         => $rifa_info["premio"],
                    "numero_ganador" => $numero_ganador,
                    "nombre_ganador" => $nombre_ganador,
                    "hay_ganador"    => ($id_usuario_ganador !== null),
                ];
                $msg_accion = "Rifa finalizada. Boleta ganadora: #" . str_pad($numero_ganador, 4, "0", STR_PAD_LEFT) . " · " . $nombre_ganador;

            } catch (Exception $e) {
                mysqli_rollback($conexion);
                $msg_accion = "Error al procesar el sorteo. Intenta de nuevo.";
            }
        } else {
...
<body class="bg-gray-100 min-h-screen flex">
    ...
    <main class="flex-1 ml-64 p-8 overflow-y-auto">
        ...
        <?php if ($msg_accion): ?>
        <div class="bg-green-100 border border-green-300 text-green-800 p-4 rounded-xl mb-4 text-sm">
            ✅ <?= htmlspecialchars($msg_accion) ?>
        </div>
        <?php endif; ?>

        <?php if ($resultado_sorteo): ?>
        <div class="bg-gradient-to-r from-yellow-400 to-orange-400 text-white rounded-2xl p-6 mb-6 shadow-lg">
            <div class="flex items-center gap-4">
                <div class="text-5xl">🏆</div>
                <div class="flex-1">
                    <p class="text-sm font-semibold opacity-90 mb-1">Sorteo ejecutado · <?= htmlspecialchars($resultado_sorteo["titulo"]) ?></p>
                    <div class="flex items-center gap-3 flex-wrap">
                        <span class="text-3xl font-black font-mono bg-white/20 rounded-xl px-4 py-1">
                            #<?= str_pad($resultado_sorteo["numero_ganador"], 4, "0", STR_PAD_LEFT) ?>
                        </span>
                        <div>
                            <?php if ($resultado_sorteo["hay_ganador"]): ?>
                            <p class="text-lg font-bold">🎉 <?= htmlspecialchars($resultado_sorteo["nombre_ganador"]) ?></p>
                            <p class="text-sm opacity-90">Premio: <?= htmlspecialchars($resultado_sorteo["premio"]) ?> · El ganador fue notificado</p>
                            <?php else: ?>
                            <p class="text-lg font-bold">Sin participantes con boletas vendidas</p>
                            <p class="text-sm opacity-90">Premio: <?= htmlspecialchars($resultado_sorteo["premio"]) ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                            ...
                            <td class="px-3 py-3">
                                <div class="flex flex-col gap-1">
                                    <a href="11_dashboard_admin.php?accion=finalizar&rid=<?= $r['id'] ?>"
                                       onclick="return confirm('¿Finalizar esta rifa y sortear el ganador aleatoriamente?')"
                                       class="text-xs bg-green-100 text-green-700 px-2 py-1 rounded hover:bg-green-200 transition-colors font-semibold text-center">
                                        🏆 Finalizar
                                    </a>
                                    <a href="11_dashboard_admin.php?accion=eliminar&rid=<?= $r['id'] ?>"
                                       ...
                                       class="text-xs bg-red-100 text-red-700 px-2 py-1 rounded hover:bg-red-200 transition-colors font-semibold text-center">
                                        🗑 Eliminar
                                    </a>
                                </div>
                            </td>
