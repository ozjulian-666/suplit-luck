...
// Notificaciones del usuario (últimas 10 no vistas — la BD no tiene columna "leida",
// usamos un truco: guardamos en sesión los IDs ya vistos)
$notificaciones = [];
$stmt_noti = mysqli_prepare($conexion,
    "SELECT id, mensaje, tipo, fecha FROM notificaciones
     WHERE id_usuario = ?
...
if ($stmt_noti) {
    ...
    $todas_notif = mysqli_fetch_all(mysqli_stmt_get_result($stmt_noti), MYSQLI_ASSOC);
    mysqli_stmt_close($stmt_noti);

    // Filtrar las que el usuario aún no ha visto (usando sesión)
    $vistas = $_SESSION["notif_vistas"] ?? [];
    foreach ($todas_notif as $n) {
        if (!in_array($n["id"], $vistas)) {
            $notificaciones[] = $n;
        }
    }
}
        ...
        <div class="flex justify-between items-center h-16">
            ...
            <div class="flex items-center space-x-3">
                ...
                <div class="relative">
                    ...
                    <span id="notif-badge"
                          class="absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center px-1 leading-none pointer-events-none">
                        <?= $notif_count ?>
                    </span>
                    <?php endif; ?>
                </div>
...
$tipo_config = [
    "resultado"    => ["emoji" => "🏆", "titulo" => "¡Resultado del sorteo!", "bg" => "bg-yellow-50", "border" => "border-yellow-400", "titulo_color" => "text-yellow-800"],
    "recordatorio" => ["emoji" => "⏰", "titulo" => "Recordatorio",           "bg" => "bg-blue-50",   "border" => "border-blue-400",   "titulo_color" => "text-blue-800"],
    "alerta"       => ["emoji" => "⚠️",  "titulo" => "Aviso del sistema",      "bg" => "bg-red-50",    "border" => "border-red-400",    "titulo_color" => "text-red-800"],
    "confirmacion" => ["emoji" => "✅",  "titulo" => "Confirmación",           "bg" => "bg-green-50",  "border" => "border-green-400",  "titulo_color" => "text-green-800"],
];
$default_cfg = ["emoji" => "🔔", "titulo" => "Notificación", "bg" => "bg-amber-50", "border" => "border-amber-400", "titulo_color" => "text-amber-800"];
?>
...
<div id="notif-panel"
    ...
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
        <div class="flex items-center gap-2">
            ...
            <?php if ($notif_count > 0): ?>
            <span id="badge-panel" class="bg-red-500 text-white text-[10px] font-bold rounded-full px-1.5 py-0.5 leading-none">
                <?= $notif_count ?>
            </span>
            <?php endif; ?>
        </div>
        <button onclick="togglePanel()" class="text-gray-400 hover:text-gray-600 text-xl leading-none">×</button>
    </div>
    ...
    <?php else: ?>
        <?php foreach ($notificaciones as $notif):
            $cfg = $tipo_config[$notif["tipo"] ?? ""] ?? $default_cfg;
        ?>
        <div data-id="<?= (int)$notif["id"] ?>"
             class="notif-item flex items-start gap-3 px-4 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors">
            <div class="flex-shrink-0 w-9 h-9 <?= $cfg["bg"] ?> rounded-xl flex items-center justify-center text-base mt-0.5">
                <?= $cfg["emoji"] ?>
            </div>
            <div class="flex-1 min-w-0">
                <p class="text-xs font-bold <?= $cfg["titulo_color"] ?> mb-0.5"><?= $cfg["titulo"] ?></p>
            ...
            <button onclick="cerrarNotif(this, <?= (int)$notif["id"] ?>)"
                    class="flex-shrink-0 text-gray-300 hover:text-red-400 transition-colors text-sm font-bold ml-1 mt-0.5"
...
<div id="notif-overlay" class="hidden fixed inset-0 z-40" onclick="togglePanel()"></div>
    ...
    setTimeout(() => {
        ...
        if (notifCount > 0) {
            ...
            document.getElementById("notif-list").innerHTML =
                '<div class="text-center py-10 text-gray-400"><div class="text-4xl mb-2">🔕</div><p class="text-sm">Sin notificaciones nuevas</p></div>';
        }
    }, 250);
    // Guardar en sesión vía fetch para no volver a mostrarla
    fetch("marcar_notif_vista.php?id=" + id);
}
</script>
</body>
</html>
