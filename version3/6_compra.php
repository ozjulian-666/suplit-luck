        ...
        try {
            ...
            if (count($boletas_ok) < $qty_real) {
                ...
                if ($filas === 0) {
                    ...
                    // ── SORTEO AUTOMÁTICO: verificar si se agotaron todas las boletas ──────
                    $stmt_check_llena = mysqli_prepare($conexion,
                        "SELECT COUNT(*) as disponibles FROM boletas WHERE id_rifa = ? AND estado = 'disponible'"
                    );
                    mysqli_stmt_bind_param($stmt_check_llena, "i", $id_rifa);
                    mysqli_stmt_execute($stmt_check_llena);
                    $disponibles_restantes = (int)mysqli_fetch_assoc(mysqli_stmt_get_result($stmt_check_llena))["disponibles"];
                    mysqli_stmt_close($stmt_check_llena);

                    if ($disponibles_restantes === 0) {
                        // Verificar que no exista ya un sorteo ejecutado para esta rifa
                        $stmt_check_sorteo = mysqli_prepare($conexion,
                            "SELECT id FROM sorteos WHERE id_rifa = ? AND estado = 'ejecutado' LIMIT 1"
                        );
                        mysqli_stmt_bind_param($stmt_check_sorteo, "i", $id_rifa);
                        mysqli_stmt_execute($stmt_check_sorteo);
                        $sorteo_ya_ejecutado = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt_check_sorteo));
                        mysqli_stmt_close($stmt_check_sorteo);

                        if (!$sorteo_ya_ejecutado) {
                            // Obtener todas las boletas vendidas de esta rifa
                            $stmt_boletas_vendidas = mysqli_prepare($conexion,
                                "SELECT b.id AS id_boleta, b.numero, b.id_usuario
                                 FROM boletas b
                                 WHERE b.id_rifa = ? AND b.estado = 'vendida' AND b.id_usuario IS NOT NULL"
                            );
                            mysqli_stmt_bind_param($stmt_boletas_vendidas, "i", $id_rifa);
                            mysqli_stmt_execute($stmt_boletas_vendidas);
                            $boletas_vendidas = mysqli_fetch_all(mysqli_stmt_get_result($stmt_boletas_vendidas), MYSQLI_ASSOC);
                            mysqli_stmt_close($stmt_boletas_vendidas);

                            if (!empty($boletas_vendidas)) {
                                // Elegir ganador aleatoriamente
                                $indice_ganador  = array_rand($boletas_vendidas);
                                $boleta_ganadora = $boletas_vendidas[$indice_ganador];

                                mysqli_begin_transaction($conexion);
                                try {
                                    // 1. Registrar o actualizar sorteo como ejecutado
                                    $stmt_sorteo_exist = mysqli_prepare($conexion,
                                        "SELECT id FROM sorteos WHERE id_rifa = ? LIMIT 1"
                                    );
                                    mysqli_stmt_bind_param($stmt_sorteo_exist, "i", $id_rifa);
                                    mysqli_stmt_execute($stmt_sorteo_exist);
                                    $sorteo_row = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt_sorteo_exist));
                                    mysqli_stmt_close($stmt_sorteo_exist);

                                    if ($sorteo_row) {
                                        $id_sorteo = (int)$sorteo_row["id"];
                                        $stmt_upd_sorteo = mysqli_prepare($conexion,
                                            "UPDATE sorteos SET numero_ganador = ?, estado = 'ejecutado', fecha_sorteo = NOW() WHERE id = ?"
                                        );
                                        mysqli_stmt_bind_param($stmt_upd_sorteo, "ii", $boleta_ganadora["numero"], $id_sorteo);
                                        mysqli_stmt_execute($stmt_upd_sorteo);
                                        mysqli_stmt_close($stmt_upd_sorteo);
                                    } else {
                                        $stmt_ins_sorteo = mysqli_prepare($conexion,
                                            "INSERT INTO sorteos (id_rifa, fecha_sorteo, numero_ganador, estado) VALUES (?, NOW(), ?, 'ejecutado')"
                                        );
                                        mysqli_stmt_bind_param($stmt_ins_sorteo, "ii", $id_rifa, $boleta_ganadora["numero"]);
                                        mysqli_stmt_execute($stmt_ins_sorteo);
                                        $id_sorteo = (int)mysqli_stmt_insert_id($stmt_ins_sorteo);
                                        mysqli_stmt_close($stmt_ins_sorteo);
                                    }

                                    // 2. Registrar en tabla ganadores
                                    $stmt_ganador = mysqli_prepare($conexion,
                                        "INSERT INTO ganadores (id_sorteo, id_usuario, id_boleta) VALUES (?, ?, ?)"
                                    );
                                    mysqli_stmt_bind_param($stmt_ganador, "iii",
                                        $id_sorteo,
                                        $boleta_ganadora["id_usuario"],
                                        $boleta_ganadora["id_boleta"]
                                    );
                                    mysqli_stmt_execute($stmt_ganador);
                                    mysqli_stmt_close($stmt_ganador);

                                    // 3. Marcar rifa como finalizada
                                    $stmt_fin_rifa = mysqli_prepare($conexion,
                                        "UPDATE rifas SET estado = 'finalizada' WHERE id = ?"
                                    );
                                    mysqli_stmt_bind_param($stmt_fin_rifa, "i", $id_rifa);
                                    mysqli_stmt_execute($stmt_fin_rifa);
                                    mysqli_stmt_close($stmt_fin_rifa);

                                    // 4. Notificar al ganador
                                    $msg_ganador  = "🎉 ¡Felicitaciones! Ganaste la rifa: " . $rifa["titulo"] . ". Tu boleta #" . str_pad($boleta_ganadora["numero"], 4, "0", STR_PAD_LEFT) . " fue la ganadora.";
                                    $tipo_gan     = "resultado";
                                    $stmt_notif_g = mysqli_prepare($conexion,
                                        "INSERT INTO notificaciones (id_usuario, id_rifa, mensaje, tipo) VALUES (?, ?, ?, ?)"
                                    );
                                    mysqli_stmt_bind_param($stmt_notif_g, "iiss",
                                        $boleta_ganadora["id_usuario"],
                                        $id_rifa,
                                        $msg_ganador,
                                        $tipo_gan
                                    );
                                    mysqli_stmt_execute($stmt_notif_g);
                                    mysqli_stmt_close($stmt_notif_g);

                                    // 5. Notificar al organizador
                                    $msg_org_fin   = "✅ Tu rifa \"" . $rifa["titulo"] . "\" se completó. Ganador: boleta #" . str_pad($boleta_ganadora["numero"], 4, "0", STR_PAD_LEFT) . ".";
                                    $tipo_org_fin  = "alerta";
                                    $stmt_notif_o  = mysqli_prepare($conexion,
                                        "INSERT INTO notificaciones (id_usuario, id_rifa, mensaje, tipo) VALUES (?, ?, ?, ?)"
                                    );
                                    mysqli_stmt_bind_param($stmt_notif_o, "iiss",
                                        (int)$rifa["id_organizador"],
                                        $id_rifa,
                                        $msg_org_fin,
                                        $tipo_org_fin
                                    );
                                    mysqli_stmt_execute($stmt_notif_o);
                                    mysqli_stmt_close($stmt_notif_o);

                                    mysqli_commit($conexion);

                                    // Guardar info del ganador en sesión para mostrar en confirmación
                                    $_SESSION["sorteo_automatico"] = [
                                        "ejecutado"      => true,
                                        "rifa_titulo"    => $rifa["titulo"],
                                        "numero_ganador" => $boleta_ganadora["numero"],
                                        "soy_ganador"    => ((int)$boleta_ganadora["id_usuario"] === $id_usuario),
                                    ];

                                } catch (Exception $e) {
                                    mysqli_rollback($conexion);
                                    // El sorteo falló pero la compra ya fue confirmada — continuar sin error
                                }
                            }
                        }
                    }
                    // ── FIN SORTEO AUTOMÁTICO ─────────────────────────────────────────────
